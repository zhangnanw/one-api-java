...
class RelayCoordinatorTest {
    ...
    void byPrefPrimary_lowerPrefComesFirst() {
        var a = rv(1, 0.1f, "free", InstanceRepo.STATUS_RAW);
        var b = rv(2, 0.5f, "free", InstanceRepo.STATUS_RAW);
    ...
    void layerOffset_freeBeforePayg() {
        ...
        var free = rv(1, 0.8f, "free", InstanceRepo.STATUS_RAW);
        var payg = rv(2, 0.001f, "payg", InstanceRepo.STATUS_RAW);
    ...
    void layerOffset_subscriptionBeforePayg() {
        ...
        var sub = rv(1, 0.5f, "subscription", InstanceRepo.STATUS_RAW);
        var payg = rv(2, 0.02f, "payg", InstanceRepo.STATUS_RAW);
    ...
    void samePref_statusDescTiebreak() {
        var raw = rv(1, 0.5f, "free", InstanceRepo.STATUS_RAW);
        var tagged = rv(2, 0.5f, "free", InstanceRepo.STATUS_TAGGED);
    ...
    void unknownLayer_treatedAsPayg() {
        var a = rv(1, 0.1f, null, InstanceRepo.STATUS_RAW);
        var b = rv(2, 0.1f, "payg", InstanceRepo.STATUS_RAW);
    ...
    void equalAllFields_stableOrder() {
        var a = rv(1, 0.5f, "free", InstanceRepo.STATUS_RAW);
        var b = rv(2, 0.5f, "free", InstanceRepo.STATUS_RAW);
    ...
    void sortList_verifiesOrdering() {
        var list = new ArrayList<>(List.of(
            rv(1, 0.5f, "payg", InstanceRepo.STATUS_RAW),           // 2.5
            rv(2, 0.3f, "subscription", InstanceRepo.STATUS_RAW),   // 1.3
            rv(3, 0.1f, "free", InstanceRepo.STATUS_TAGGED),        // 0.1, TAGGED
            rv(4, 0.1f, "free", InstanceRepo.STATUS_RAW)            // 0.1, RAW
        ));
        list.sort(sorter);
