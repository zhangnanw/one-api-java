 ...
 * NameMatcher 单元测试。
 *
 * 核心行为：物理模型名（出现在 instances 表）只在 requireVirtualModel=false
 * 时直接打 matchedPhysical 标记。严格模式（默认）下不设任何字段，让后续
 * VirtualModelLookup filter 决定——这是"API 表面只暴露虚拟模型"的关键。
 */
...
class NameMatcherTest {
    ...
    void strict_skipsMatchedPhysicalMark() {
        ...
        new NameMatcher(instanceRepo, true).apply(ctx);
    ...
    @Test
    void nonStrict_marksMatchedPhysical() {
        when(instanceRepo.existsByModelName(PHYSICAL_MODEL)).thenReturn(true);

        RelayContext ctx = TestFixtures.relayContext(null, null, PHYSICAL_MODEL);
        new NameMatcher(instanceRepo, false).apply(ctx);

        // 非严格模式：filter 直接放过
        assertThat(ctx.matchedPhysical()).isTrue();
        assertThat(ctx.upstreamModel()).isEqualTo(PHYSICAL_MODEL);
